...
macOS on Apple Silicon or Intel; Python 3.12+, git/curl/jq/gh. Additional tools depend on
the selected [workload capabilities](../../docs/WORKLOADS.md); Ruby is not required for generic/Node/backend.
...
| schema_version / capabilities | v2: selected workload checks; legacy mobile configs are still read |
| versions / required_tools | Selected version constraints and additional executable names |
...
- SDK (Android only): `/Users/<ci_user>/Library/Android/sdk`;
- CI shell environment: `~/.config/local-ci/env.sh`, generated by `shell-env` and sourced by `.zprofile`.
...
## Requirement changes and Docker

Review configuration while stopped, then repeat setup from the updated source. Legacy input
is normalized into a private v2 installation snapshot; the source file and matching registration
...
CI, not owner preflight; missing CI tools can leave setup installed but incomplete. Prepare
them under CI and rerun doctor. See the workload contract for exact `--version` limitations.
